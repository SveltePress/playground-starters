...
- `custom-theme` — newly authored Custom theme starter: no Default Theme, no Blog theme, layout authored in the project. English opens `src/routes/+layout.svelte`; Chinese and Bengali open `src/routes/zh/+layout.svelte` and `src/routes/bn/+layout.svelte` (translated chrome, not copies of the English layout).
